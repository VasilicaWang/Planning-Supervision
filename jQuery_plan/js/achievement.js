$(function () {
    ...
    function run(n) {
            ...
            if (n == 0) {
                if (val == today) {
                    ...
                    var time = setInterval(function () {
                        ...
                        if (nowYear == todayYear && nowMonth == todayMonth && nowDay == todayDay && nowHour == 14 && nowMinute == 39 && nowSecond == 0) {
                            console.log("ok");
                            // 获取数据库百分数以及计划项目数
                            axios.post("http://localhost:3000/getPerandlist", {
                                userName: $('#userName').text()
                            }).then(res => {
                                var percent = res.data.percent; //percent：获取到的百分数
                                var listNum = res.data.listNum; //listNum：获取到计划数目
        ...
        // 获取数据库成就页相关数据添加到成就界面
        axios.post("http://localhost:3000/getAchi", {
            userName: $('#userName').text()
        }).then(res => {
